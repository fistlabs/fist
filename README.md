...
Метод, который выполняет ответ приложения и останавливает реакцию разрешения узла.
...
Также ```send``` понимает буфферы и ```Readable```-потоки. Если не был выставлен ```Content-Type```, то ```send``` подумает что это абстрактный бинарник и поставит ```Content-Type: application/octet-stream```. ```send``` особенным образом относится к объектам класса ```Error```. Если вы НЕ запустили приложение с параметром ```staging: true```, то если вы будете передавать в ```send``` объекты ошибок со statusCode >= 500, то клиенту будет отправлен ```stack-trace```, иначе вместо ```stack-trace``` будет отправлено стандартное http-сообщение, соответствующее ```statusCode```. Также send заботится о том чтобы не было отправлено тело в ответах на ```HEAD```-запросы, или если вы установили специфический ```statusCode```, который не предполагает наличие тела. Например ```304``` (Not Modified), в таком случае send еще позаботится о том, чтобы на клиент не ушли conent-* заголовки.
####```track.invoke(name, done)```
Запускает операцию разрешения на [узле](#%D0%A3%D0%B7%D0%B5%D0%BB) по его имени. Вторым аргументом принимает коллбэк, который будет вызван кк обычно с одним или двумя аргументами в зависимости от того случилась ошибка или нет.
####```track.buildPath(name[, params])```
Создает ```url``` страницы приложения по ```name``` маршрута из ```expr```, указанном при декларации маршрута. Вторым аргументом передаются параметры для ```url```. Часть параметром попадет в path ```url```-a, в зависимости от того какой ```expr```, а остальные попадут в ```query```.
