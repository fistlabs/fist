...
#TODO (0.0.1)
 1. Рефакторинг BodyParser, сделать возможным добавление кастомных парсеров определяющихся по content-type
 2. Подумать о нескольких роутерах на одном сервере, если будет необходимо и ясно, то запланировать на 1.0.0
 3. Методы util/caller protected сделать членами Framework (будет неудобно тестировать)
